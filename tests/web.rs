...
fn test_mask_text_with_fields_basic() {
    ...
    assert_eq!(
        result, expected,
        "Words should be replaced with FIELD_N format based on length order"
    );
}
...
fn test_mask_text_with_fields_multiple_occurrences() {
    ...
    assert_eq!(
        result, expected,
        "Same words should use same field reference"
    );
}
...
fn test_decode_obfuscated_text_basic() {
    ...
    assert_eq!(
        result, expected,
        "FIELD_N should be replaced with corresponding words with correct casing"
    );
}
...
fn test_decode_obfuscated_text_no_fields() {
    ...
    assert_eq!(
        result, expected,
        "Text without fields should remain unchanged"
    );
}
...
fn test_decode_obfuscated_text_empty_words() {
    ...
    assert_eq!(
        result, expected,
        "Empty words should be skipped during field mapping"
    );
}
...
fn test_mask_and_decode_roundtrip() {
    ...
    assert_eq!(
        masked, "My FIELD_1 is admin and my FIELD_2 is 12345.",
...
fn test_case_preservation_variations() {
    ...
    assert_eq!(
        masked_lower, "this is a FIELD_1 message",
    ...
    assert_eq!(
        masked_title, "this is a FIELD_1_F message",
    ...
    assert_eq!(
        masked_upper, "this is a FIELD_1_A message",
...
fn test_mask_text_substring_words() {
    ...
    assert_eq!(
        result, expected,
        "Longer words containing shorter mask words should be masked correctly"
    );
    ...
    assert_eq!(
        ...
        "Field masking should handle substring words correctly"
    );
}
...
fn test_decode_obfuscated_text_mask_word_casing() {
    ...
    assert_eq!(
        result, expected,
        "Decoding should properly handle casing regardless of mask word casing"
    );
}
...
fn test_mask_text_with_compound_words() {
    ...
    assert_eq!(
        result, expected,
        "Compound words should be partially masked"
    );
}
...
fn test_mask_text_with_fields_compound_words() {
    ...
    assert_eq!(
        result, expected,
        "Compound words should be properly masked with fields"
    );
}
...
fn test_mask_text_with_fields_camel_case() {
    ...
    assert_eq!(
        result, expected,
        "CamelCase words should be properly masked with fields"
    );
}
...
fn test_mask_text_with_fields_snake_case() {
    ...
    assert_eq!(
        result, expected,
        "Snake_case words should be properly masked with fields"
    );
}
...
fn test_mask_text_with_fields_mixed_compound_types() {
    ...
    assert_eq!(
        result, expected,
        "Mixed compound word types should be properly masked"
    );
}
...
fn test_decode_obfuscated_text_with_compound_words() {
    ...
    assert_eq!(
        result, expected,
        "Compound words with fields should be properly decoded"
    );
}
...
fn test_decode_compound_words_with_underscores() {
    ...
    assert_eq!(
        result, expected,
        "Fields in snake_case words should be properly decoded"
    );
}
...
fn test_decode_compound_words_with_camel_case() {
    ...
    assert_eq!(
        result, expected,
        "Fields in camelCase words should be properly decoded"
    );
}
...
fn test_decode_compound_words_mixed_formats() {
    ...
    assert_eq!(
        result, expected,
        "Fields in mixed formats should be properly decoded"
    );
}
...
fn test_roundtrip_compound_words() {
    ...
    assert_eq!(
        result, expected,
        "Roundtrip masking and decoding should preserve the original text"
    );
}
...
fn test_mask_decode_compound_camel_case() {
    ...
    let masked = mask_text_with_fields(original.to_string(), &mask_words);
    assert_eq!(
        masked, "myFIELD_2_FFIELD_1_F FIELD_3FIELD_1_FKey FIELD_2FIELD_3_FHandler",
...
fn test_mask_decode_compound_underscore() {
    ...
    let masked = mask_text_with_fields(original.to_string(), &mask_words);
    assert_eq!(
        masked, "my_FIELD_2_FIELD_1 FIELD_3_FIELD_1_key FIELD_2_FIELD_3_config",
...
fn test_mask_decode_mixed_compound_patterns() {
    ...
    let masked = mask_text_with_fields(original.to_string(), &mask_words);
    assert_eq!(
        masked, "myFIELD_2_FFIELD_1_F_FIELD_3 FIELD_2_FIELD_3FIELD_1_F FIELD_3_A_FIELD_1_A_KEY",
